  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (item.status == "success") {
        $("#file-result").append(
          `<li class="list-group-item" data-path="${item.path}"><button id="document" class="btn btn-link">${category}</button></li>`
        );
...
$("#product-register").on("submit", function (e) {
  e.preventDefault();
  const formData = $(this).serialize();
  $.post("/client/register", formData, function (response) {
